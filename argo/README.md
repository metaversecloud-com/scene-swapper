...
**Secrets:** no plaintext committed. `scene-sw0-sealedsecret.yaml` holds only ciphertext
(sealed strict-scope for namespace sdk-apps-dev); the controller unseals it into
`Secret scene-sw0-secrets`, consumed via `envFrom`. Non-secret env (incl. public
`INTERACTIVE_KEY`) lives in `scene-sw0-config`.

## Environment

| Env | Service | Namespace | Host | Health |
| --- | ------- | --------- | ---- | ------ |
| `dev` | `scene-sw0` | `sdk-apps-dev` | scene-sw0-dev-topia.topia-rtsdk.com | `/api/system/health` |
